...
转载于 链接：https://pdai.tech/md/java/jvm/java-jvm-gc-g1.html
...
![img](/_images/java/java-jvm-gc-g1-1.jpeg)

#### [¶](#211-分区region) 2.1.1 分区Region

G1采用了分区(Region)的思路，将整个堆空间分成若干个大小相等的内存区域，每次分配对象空间将逐段地使用内存。因此，在堆的使用上，G1并不要求对象的存储一定是物理上连续的，只要逻辑上连续即可；每个分区也不会确定地为某个代服务，可以按需在年轻代和老年代之间切换。启动时可以通过参数-XX:G1HeapRegionSize=n可指定分区大小(1MB~32MB，且必须是2的幂)，默认将整堆划分为2048个分区。

#### [¶](#212-卡片card) 2.1.2 卡片Card

在每个分区内部又被分成了若干个大小为512 Byte卡片(Card)，标识堆内存最小可用粒度所有分区的卡片将会记录在全局卡片表(Global Card Table)中，分配的对象会占用物理上连续的若干个卡片，当查找对分区内对象的引用时便可通过记录卡片来查找该引用对象(见RSet)。每次对内存的回收，都是对指定分区的卡片进行处理。

#### [¶](#213-堆heap) 2.1.3 堆Heap

G1同样可以通过-Xms/-Xmx来指定堆空间大小。当发生年轻代收集或混合收集时，通过计算GC与应用的耗费时间比，自动调整堆空间大小。如果GC频率太高，则通过增加堆尺寸，来减少GC频率，相应地GC占用的时间也随之降低；目标参数-XX:GCTimeRatio即为GC与应用的耗费时间比，G1默认为9，而CMS默认为99，因为CMS的设计原则是耗费在GC上的时间尽可能的少。另外，当空间不足，如对象空间分配或转移失败时，G1会首先尝试增加堆空间，如果扩容失败，则发起担保的Full GC。Full GC后，堆尺寸计算结果也会调整堆空间。

### [¶](#22-分代模型) 2.2 分代模型

![img](/_images/java/java-jvm-gc-g1-2.jpeg)

#### [¶](#221-分代垃圾收集) 2.2.1 分代垃圾收集
...
#### [¶](#222-本地分配缓冲-local-allocation-buffer-lab) 2.2.2 本地分配缓冲 Local allocation buffer (Lab)
...
![img](/_images/java/java-jvm-gc-g1-3.jpeg)

G1对内存的使用以分区(Region)为单位，而对对象的分配则以卡片(Card)为单位。

#### [¶](#231-巨形对象humongous-region) 2.3.1 巨形对象Humongous Region
...
#### [¶](#232-已记忆集合remember-set-rset) 2.3.2 已记忆集合Remember Set (RSet)
...
#### [¶](#233-per-region-table-prt) 2.3.3 Per Region Table (PRT)
...
### [¶](#24-收集集合-cset) 2.4 收集集合 (CSet)

CSet收集示意图

![img](/_images/java/java-jvm-gc-g1-4.jpeg)
...
![img](/_images/java/java-jvm-gc-g1-5.png)
...
![img](/_images/java/java-jvm-gc-g1-5-1.gif)

#### [¶](#244-漏标问题) 2.4.4 漏标问题

在remark过程中，黑色指向了白色，如果不对黑色重新扫描，则会漏标。会把白色D对象当作没有新引用指向从而回收掉。

![img](/_images/java/java-jvm-gc-g1-6.png)
...
### [¶](#31-g1垃圾收集活动汇总) 3.1 G1垃圾收集活动汇总

G1垃圾收集活动周期图

![img](/_images/java/java-jvm-gc-g1-7.jpeg)

### [¶](#32-rset的维护) 3.2 RSet的维护
...
#### [¶](#321-栅栏barrier) 3.2.1 栅栏Barrier

栅栏代码示意

![img](/_images/java/java-jvm-gc-g1-8.jpeg)
...
#### [¶](#322-起始快照算法snapshot-at-the-beginning-satb) 3.2.2 起始快照算法Snapshot at the beginning (SATB)
...
#### [¶](#323-并发优化线程concurrence-refinement-threads) 3.2.3 并发优化线程Concurrence Refinement Threads
...
### [¶](#33-并发标记周期-concurrent-marking-cycle) 3.3 并发标记周期 Concurrent Marking Cycle
...
#### [¶](#331-并发标记线程-concurrent-marking-threads) 3.3.1 并发标记线程 Concurrent Marking Threads

并发标记位图过程

![img](/_images/java/java-jvm-gc-g1-9.png)
...
#### [¶](#332-初始标记-initial-mark) 3.3.2 初始标记 Initial Mark
...
#### [¶](#333-根分区扫描-root-region-scanning) 3.3.3 根分区扫描 Root Region Scanning
...
#### [¶](#334-并发标记-concurrent-marking) 3.3.4 并发标记 Concurrent Marking
...
#### [¶](#337-清除-cleanup) 3.3.7 清除 Cleanup
...
### [¶](#34-年轻代收集混合收集周期) 3.4 年轻代收集/混合收集周期
...
### [¶](#35-并发标记周期后的年轻代收集-young-collection-following-concurrent-marking-cycle) 3.5 并发标记周期后的年轻代收集 Young Collection Following Concurrent Marking Cycle

当G1发起并发标记周期之后，并不会马上开始混合收集。G1会先等待下一次年轻代收集，然后在该收集阶段中，确定下次混合收集的CSet(Choose CSet)。

#### [¶](#351-混合收集周期-mixed-collection-cycle) 3.5.1 混合收集周期 Mixed Collection Cycle
...
#### [¶](#352-转移失败的担保机制-full-gc) 3.5.2 转移失败的担保机制 Full GC
