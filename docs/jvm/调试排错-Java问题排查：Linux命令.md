...
转载于 链接：https://pdai.tech/md/java/jvm/java-jvm-debug-tools-linux.html
...
```bash
[root.tech ~]# netstat -nat|awk  '{print $6}'|sort|uniq -c|sort -rn
      5 LISTEN
      2 ESTABLISHED
      1 Foreign
      1 established)
...
```bash
[root.tech ~]# grep MemTotal /proc/meminfo
MemTotal:        1882088 kB
...
```bash
[root.tech ~]# grep MemFree /proc/meminfo
MemFree:           74120 kB
...
### [¶](#查看系统负载磁盘和分区-2) 查看系统负载磁盘和分区

```bash
[root.tech ~]# cat /proc/loadavg
