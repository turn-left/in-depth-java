...
![img](/docs/jvm/imgs/20200309181645.png)

应用故障诊断思路：

![img](/docs/jvm/imgs/20200309181831.png)
...
tcp 队列溢出是个相对底层的错误，它可能会造成超时、rst 等更表层的错误。因此错误也更隐蔽，所以我们单独说一说。 ![img](/docs/jvm/imgs/2019-11-04-083827.jpg)
...
**netstat 命令，执行 netstat -s | egrep "listen|LISTEN"** ![img](/docs/jvm/imgs/2019-11-04-83828.jpg) 如上图所示，overflowed 表示全连接队列溢出的次数，sockets dropped 表示半连接队列溢出的次数。

**ss 命令，执行 ss -lnt** ![img](/docs/jvm/imgs/2019-11-04-083828.jpg) 上面看到 Send-Q 表示第三列的 listen 端口上的全连接队列最大为 5，第一列 Recv-Q 为全连接队列当前使用了多少。
...
我们在诊断故障时候怎么确定有 RST 包的存在呢？当然是使用 tcpdump 命令进行抓包，并使用 wireshark 进行简单分析了。`tcpdump -i en0 tcp -w xxx.cap`，en0 表示监听的网卡。 ![img](/docs/jvm/imgs/2019-11-04-083829.jpg)

接下来我们通过 wireshark 打开抓到的包，可能就能看到如下图所示，红色的就表示 RST 包了。 ![img](/docs/jvm/imgs/2019-11-04-083830.jpg)
...
![img](/docs/jvm/imgs/2019-11-04-083830.png)
...
> OOM 定位可以参考：[调试排错-Java-OOM分析](/docs/jvm/调试排错-Java-OOM分析.md)
...
- [JAVA 线上故障诊断全套路(opens new window)](https://fredal.xin/java-error-check)
